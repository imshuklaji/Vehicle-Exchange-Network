...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      echo "ERROR! Local Fabric binary version of $LOCAL_VERSION does not match this newer version of vehicle-exchange-network and is unsupported. Either move to a later version of Fabric or checkout an earlier version of vehicle-exchange-network."
      exit 1
    fi

    echo "$DOCKER_IMAGE_VERSION" | grep -q "$UNSUPPORTED_VERSION"
    if [ $? -eq 0 ]; then
      echo "ERROR! Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match this newer version of vehicle-exchange-network and is unsupported. Either move to a later version of Fabric or checkout an earlier version of vehicle-exchange-network."
      exit 1
    fi
  done
}
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/manufacturer.vehicle-exchange-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/manufacturer_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/individual1.vehicle-exchange-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/individual1_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/individual2.vehicle-exchange-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/individual2_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/individual3.vehicle-exchange-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/individual3_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile OrdererGenesis -channelID individual1-sys-channel -outputBlock ./channel-artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile vehicleexchangechannel -outputAnchorPeersUpdate ./channel-artifacts/manufacturerMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg manufacturerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for individual1MSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile vehicleexchangechannel -outputAnchorPeersUpdate ./channel-artifacts/individual1MSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg individual1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for individual1..."
    exit 1
  fi
  echo
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for individual2MSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile vehicleexchangechannel -outputAnchorPeersUpdate ./channel-artifacts/individual2MSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg individual2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for individual2..."
    exit 1
  fi
  echo
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for individual3MSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile vehicleexchangechannel -outputAnchorPeersUpdate ./channel-artifacts/individual3MSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg individual3MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for individual3..."
    exit 1
  fi
  echo
